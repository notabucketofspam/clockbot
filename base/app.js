...
import { Client, Events, GatewayIntentBits, VoiceChannel, SlashCommandBuilder, Collection, MessageFlags } from "discord.js";
...
client.on(Events.InteractionCreate, async (interaction) => {
    if (interaction.isCommand()) {
        const com = commands.get(interaction.commandName);
        if (com) {
            try {
...
*/
import { joinVoiceChannel, createAudioPlayer, createAudioResource, StreamType, getVoiceConnection } from "@discordjs/voice";
const players = new Map();
/**
 *
 * @param channel_id This is the voice channel id
 */
async function getinchat(channel_id) {
 ...
 */
async function leave_chat(channel_id) {
    const channel = await client.channels.fetch(channel_id);
    if (channel instanceof VoiceChannel) {
...
*/
async function beep(channel_id, fpath) {
    const channel = await client.channels.fetch(channel_id);
    if (channel instanceof VoiceChannel) {
        if (fpath.includes("."))
            return;
